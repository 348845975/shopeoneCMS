    ...
    >
      ...
      <div class="smlBox">
        ...
        <div>
          ...
          <el-form-item label="图片" class="priPic" prop="public">
            <el-upload
              ref="upload1"
              :action="httpUrl"
              name="file"
        ...
        <div v-if="hide" class="skutable">
          ...
          >
            ...
            <el-table-column label="SKU图片" align="center" width="300">
              <template slot-scope="scope">
        ...
        <el-form-item
          ...
          prop="public"
        >
          <el-input v-model="item.form" v-if="item.pAttributeOptionsList.length==0" size="mini"
        ...
        <p style="clear:both" class="secTit">
          商品特殊说明
             <el-input
        ...
        <el-form-item>
          <el-button @click="onSubmit" type="primary" size="mini" plain>保存</el-button>
          <el-button @click="goAhead" type="primary" size="mini" plain>取消</el-button>
        </el-form-item>
      </div>
    </el-form>
  </div>
...
export default {
  ...
  methods: {
    refreshProductSkuList() {
      ...
      this.saleAttribute.forEach(ele => {
        if (ele.form.length == 0) {
          this.hide = false;
        } else if (ele.form.length > 0) {
          arr.push("1");
          if (arr.length == this.saleAttribute.length) {
            this.hide = true;
          }
        }
      });
    ...
    handleRemove() {},
    handleExceed() {
      this.$message.warning("SPU主图只能上传一张");
    },
    ...
    handleSuccessColor(res, file, fileList) {
      ...
      this.fileList = [];
      console.log(" this.ruleForm.sku:", this.ruleForm.sku[this.index]);
      if (res.code == "0") {
        let obj = {};
        obj.name = "mockName";
