  ...
  <div>
    ...
    >
      <el-form-item
        ...
        prop="type1"
      >
        <el-checkbox-group v-model="item1.form" @change="hrr(index)">
          <span v-for="item2 in item1.pAttributeOptionsList" :key="item2.id">
            <el-checkbox style="margin-right:0px;" :label="item2.id">{{item2.name}}</el-checkbox>
            <el-upload
              v-if="item1.name=='颜色'"
              class="upload-demo"
              :action="httpUrl"
            ...
            >
              <el-button
                @click="upLoadImg($event,item2.id)"
                v-if="item1.form.indexOf(item2.id)!==-1"
              >上传</el-button>
            </el-upload>
          </span>
        </el-checkbox-group>
    ...
    <div>
      ...
      <div class="specification">
        ...
        <ul class="spec-list">
          <li class="item" v-for="(item, index) in saleAttribute" :key="index">
            <div class="name">
              <el-input size="small" v-model="item.newName" placeholder="输入产品规格"></el-input>
              <i class="icon el-icon-circle-close" @click="delSpec(index)"></i>
            </div>
            <div class="values">
              <span class="el-tag" v-for="(tag, num) in item.form" :key="tag">
      ...
      <div class="example">
        ...
        <table class="stock-table">
          <thead>
            <tr>
              <th v-for="(item, index) in saleAttribute" :key="index">{{item.newName}}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in countSum(0)" :key="index">
              <td
                v-for="(n, specIndex) in saleAttribute.length"
                :key="n"
      ...
      <div class="example">
        ...
        <table class="stock-table" cellspacing="0" cellpadding="0">
          ...
          <tbody v-if="saleAttribute[0] && saleAttribute[0].form.length>0">
            <tr :key="index" v-for="(item, index) in countSum(0)">
              ...
              <td>
                <el-input
                  size="small"
                  type="text"
                  v-model="childProductArray[index].childProductStock"
                  placeholder="输入库存"
                ></el-input>
              </td>
              <td>
...
import { getCategory, getBrandAndAttribute } from "@/api/getData";
...
export default {
  ...
  computed: {
    // 所有sku的id
    stockSpecArr() {
      return this.childProductArray.map(item => item.childProductSpec); //port
    }
  },
  mounted() {
    this.initData();
  ...
  methods: {
    async initData() {
    //先不请求把数据写死了
    //   const res = await getBrandAndAttribute({
    //     token: this.$store.state.token,
    //     categoryId: this.selectedCate
    //   });
    //   console.log("res-->", res);
    //   this.saleAttribute = res.simpleJsonResult.result.data.saleAttribute;
    },
    hrr(index) {
      this.saleAttribute[index].newName = this.saleAttribute[index].name;
      console.log("this.saleAttribute", this.saleAttribute);
      this.handleSpecChange();
    //   this.stockSpecArr()
    },
    getSpecAttr(specIndex, index) {
      // 获取当前规格项目下的属性值
      const currentValues = this.saleAttribute[specIndex].form;
      let indexCopy;
      // 判断是否是最后一个规格项目
      if (
        this.saleAttribute[specIndex + 1] &&
        this.saleAttribute[specIndex + 1].form.length
      ) {
    ...
    */
    countSum(specIndex) {
      let num = 1;
      this.saleAttribute.forEach((item, index) => {
        if (index >= specIndex && item.form.length) {
          num *= item.form.length;
        }
      });
      return num;
    },
    ...
    changeStock(index, stockCopy) {
      ...
      this.childProductArray.push(childProduct);
    }
  },
  // 获取childProductArray的childProductSpec属性
  getChildProductSpec(index) {
    let obj = {};
    this.saleAttribute.forEach((item, specIndex) => {
      obj[item.newName] = this.getSpecAttr(specIndex, index);
    });
    return obj;
  },
  // 删除规格项目
